...
This link will expire in 7 days. If you didn't expect this email, please ignore it.
  ...
  try {
    ...
    const isTestEnv = process.env.NODE_ENV === 'test' || process.env.JEST_WORKER_ID || to.includes('@example.com');
    if (!isTestEnv) {
...
export async function sendContactFormEmail(name, email, company, subject, message) {
  ...
  try {
    const info = await transporter.sendMail(mailOptions);
    const isTestEnv = process.env.NODE_ENV === 'test' || process.env.JEST_WORKER_ID || process.env.API_BASE_URL;
    if (!isTestEnv) {
