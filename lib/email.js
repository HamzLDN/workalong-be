...
if (process.env.NODE_ENV !== 'test' && !process.env.JEST_WORKER_ID && !process.env.API_BASE_URL) {
  transporter.verify((error, success) => {
    if (error) {
      // Suppress errors during security tests (API_BASE_URL indicates test environment)
...
This link will expire in 7 days. If you didn't expect this email, please ignore it.
  ...
  try {
    ...
    // In test mode, silently fail (don't throw) to avoid breaking tests
    if (isTestEnv) {
...
/**
 * Send billing renewal reminder email
 * @param {string} to - User email
 * @param {string} name - User name
 * @param {number} daysLeft - Days until renewal (7, 3, or 1)
 * @param {Date} renewalDate - The renewal/expiry date
 * @param {string} billingPortalUrl - Link to update payment method
 */
...
/**
 * Send a demo booking request notification to the Workalong team.
 */
