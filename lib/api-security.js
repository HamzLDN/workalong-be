...
export async function verifyApiKey(apiKey) {
  ...
  await pool.query('UPDATE api_keys SET last_used_at = NOW() WHERE id = $1', [keyData.id]);
...
export async function getSigningKeyByHash(hash) {
  ...
  await pool.query('UPDATE request_signing_keys SET last_used_at = NOW() WHERE id = $1', [
    result.rows[0].id,
  ]);
