...
async function updatePromoCodeLimit() {
  try {
    ...
    if (args.length < 2) {
      console.error('Usage: node scripts/update-promo-code-limit.js <PROMO_CODE> <MAX_REDEMPTIONS>');
    ...
    if (promoCode.times_redeemed && promoCode.times_redeemed > maxRedemptions) {
      console.error(`❌ This code has already been redeemed ${promoCode.times_redeemed} time(s), which is > ${maxRedemptions}`);
    ...
    if (promoCode.times_redeemed && promoCode.times_redeemed === maxRedemptions) {
      console.log(`⚠️  Warning: This code has already been redeemed ${promoCode.times_redeemed} time(s).`);
      console.log(`   Setting max_redemptions to ${maxRedemptions} will prevent any future redemptions.\n`);
    }
    ...
    console.log(`Note: Stripe doesn't allow updating max_redemptions. We'll deactivate the old code and create a new one.\n`);
