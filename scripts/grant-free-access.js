...
const stripe =
  stripeSecretKey && String(stripeSecretKey).startsWith('sk_') ? new Stripe(stripeSecretKey) : null;
...
async function grantFreeAccess() {
  try {
    ...
    console.log('\n📋 Next Steps:');
    console.log(
      `1. User needs to complete payment setup at: ${subscription.latest_invoice?.hosted_invoice_url || 'N/A'}`
    );
